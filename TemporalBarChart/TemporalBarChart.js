visualizationFunctions.TemporalBarChart = function(element, data, opts) {
    ...
    context.VisFunc = function() {
        ...
        context.SVGfocus = context.config.easySVG(element[0], {
                responsive: false
            })
            .attr('background', 'white')
            .attr('class', 'focus ' + opts.ngIdentifier)
        context.SVGcontext = context.config.easySVG(element[0], {
                responsive: false
            })
            .attr('background', 'white')
        ...
        inputdata = context.filteredData.records.data;
        ...
        context.SVGfocus.append("svg:clipPath")
            .attr("id", "clipfocus")
            .append("svg:rect")
            .attr("x", 0)
            .attr("y", -(fheight))
        ...
        // context.SVGcontext.append("svg:clipPath")
        // 	.attr("id","clipcontext")
        // 	.append("svg:rect")
        // 	.attr("x", 0)
        ...
        context.SVGfocus.summaryscale = d3.scale.linear()
            .domain([d3.min(inputdata, function(d) {
                    return d['grantstartyear'];
                }),
                d3.max(inputdata, function(d) {
        ...
        context.SVGfocus.group = context.SVGfocus.append('g')
            .attr('class', 'focusgroup')
            .attr('clip-path', "url(#clipfocus)")

        // Group to append all items in the Context SVG ...
        context.SVGcontext.group = context.SVGcontext.append('g')
        ...
        context.SVGfocus.group.append("svg:clipPath")
            .attr("id", "cliprects")
            .append("svg:rect")
            .attr("x", margin)
        ...
        var focusaxis = context.SVGfocus.group.append('g')
            .attr('class', 'focusaxis')
            .attr('clip-path', "url(#clipfocus)")
            .call(fxaxis)
            .style("stroke-dasharray", ("3, 3"))

        var contextaxis = context.SVGcontext.group.append('g')
            .attr("transform", function() {
        ...
        focusallitemsgroup = context.SVGfocus.group.append('g')
        ...
        focusrects
            ...
            .attr('fill', function(d, i) {
                return context.Scales.colors(d['agency']);
            })
            .attr("opacity", 0.7)
            .attr("stroke", "black")
        ...
        contextallitemsgroup = context.SVGcontext.group.append('g')
        ...
        footeritems = context.SVGcontext.append('g')
        ...
        context.SVGfocus.brush = context.SVGfocus.append('g')
            .attr('class', 'brushgroup')
            .attr('transform', function() {
                return "translate(" + (fwidth + margin) + ",0)"
            })

        context.SVGfocus.brush.append("svg:clipPath")
        ...
        var fbrush = context.SVGfocus.brush.append('g')
        ...
        // View Summary group ...
        var viewgroup = context.SVGcontext.append('g')
        ...
        var t1 = context.SVGfocus.summaryscale.domain()[0]
        var t2 = context.SVGfocus.summaryscale.domain()[1]

        viewbox.append("foreignObject")
            .attr('class', 'viewsummary')
            .attr('width', (context.config.dims.fixedWidth) - (fwidth + (3 * margin)))
        ...
        function initBrush() {
            ...
            gBrush = context.SVGcontext.append("g")
                .attr("class", "brush")
                .attr("transform", function() {
        ...
        function update() {
            ...
            context.SVGfocus.summaryscale.domain(brush.empty() ? x2scale.domain() : brush.extent());
            var t1 = context.SVGfocus.summaryscale.domain()[0];
            var t2 = context.SVGfocus.summaryscale.domain()[1];
            d3.selectAll(".fbars")
            ...
            context.SVGfocus.group.selectAll('.focusrectgroup')
                .attr('transform', function(d, i) {
                    var t = d3.transform(d3.select(this).attr('transform')).translate[1]
                    return "translate(" + xscale(d['grantstartyear']) + "," + t + ")"
                })
            context.SVGfocus.group.selectAll(".focusaxis").call(fxaxis);
            context.SVGcontext.select(".viewsummary")
                .html(function() {
                    var vr = grantsviewbox(t1, t2)
                    return "<p><b>View Summary: </b><br /><br />" + " - # of Awards: <b>" + vr[0] + "</b><br /><br /> - Funding: <b>$" + numberToCurrency(parseInt(vr[1])) + "</b><br /><br /> - # of PI's: <b>" + "........." +
                        "</b><br /><br /> - # of Agencies <b>" + vr[2] + "</b></p>"
        ...
        function initBrush2() {
            ...
            g2Brush = context.SVGfocus.brush.append("g")
                .attr("class", "brush2")
                .call(brush2);
        ...
        function initCanvas() {
            contextwth = context.config.dims.fixedWidth
            width = ((context.config.dims.fixedWidth * 82.5) / 100)
            fwidth = width * 88 / 100
            fheight = (((context.config.dims.fixedHeight) * 70) / 100)
            cheight = ((context.config.dims.fixedHeight) * 25 / 100)
            cbrushdims = cheight / 2
            c2width = width * 9 / 100
            margin = ((context.config.dims.fixedWidth) * 1 / 100)
            console.log("dimensions", context.config.dims.fixedWidth, context.config.dims.fixedHeight, margin)
            var fcanvas = context.SVGfocus.node().parentNode
            fcanvas.setAttribute('height', fheight)
            fcanvas.setAttribute('width', width)
            var ccanvas = context.SVGcontext.node().parentNode
            ccanvas.setAttribute('height', cbrushdims + 150)
            ccanvas.setAttribute('width', context.config.dims.fixedWidth)

            context.SVGfocus.attr('transform', "translate(" + margin + ',' + (fheight - margin) + ")")
            context.SVGcontext.attr('transform', "translate(" + margin + ',' + (cheight - margin) + ")")

            // View summary box setup...
            d3.select('.canvas').attr('class', 'focusSVG')
            $('.legdiv').empty();
            context.SVGlegend = d3.select(".legdiv")
                .append('svg')
            ...
            d.style.top = (context.config.dims.fixedHeight * 2.5 / 100) + 'px';
            d.style.height = fheight - margin + "px";
            d.style.width = (context.config.dims.fixedWidth - width - margin) + "px"

            // Set the standard variable values here ...
            rectpad = 0
            padding = 22
            context.Scales.colors = d3.scale.category10();
        }

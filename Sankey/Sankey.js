...
visualizationFunctions.Sankey = function(element, data, opts) {
    ...
    this.VisFunc = function() {
        ...
        function createColumnLabels() {
            return context.SVG.group.append("g").selectAll(".selects")
                .data(context.config.meta.other.categories)
                .enter()
                ...
                .attr("y", -40)
                .html(function(d, i) {
                    var diffArr = context.config.meta.other.allcategories.diff(context.config.meta.other.categories);
                    diffArr = diffArr.concat([d]);
                    var str = "<select>";
                    diffArr.forEach(function(d1, i1) {
                        var selected = (i1 == 0) ? "" : " selected ";
                        str += "<option " + selected + "value='" + d1 + "'>" + d1 + "</option>"
                    })
                    str += "</select>"
                    return str;
                }).attr("x", function(d, i) {
                    var currNodeData = d3.select(context.SVG.selectAll(".col-" + i)[0][0]).data()[0]
                    var thisWidth = d3.select(this).selectAll("select").style("width").replace("px", "")
                    if (i == 0) return currNodeData.x;
                    if (i == context.config.meta.other.categories.length - 1) return currNodeData.x - (thisWidth) + sankey.nodeWidth();
                    return currNodeData.x - (thisWidth / 2) + (sankey.nodeWidth() / 2);
                }).on("change", function(d, i) {
                    context.config.meta.other.categories[i] = $($(this).find("select")[0]).val();
                    context.ResetVis();
                })
        }
        ...
        function applySVGEvents() {
            context.SVG.nodes.selectAll("rect")
                .on("mouseover", function(d) {
                    ...
                    context.SVG.edges.filter("[class*='" + d.name + "-']").moveToFront().classed("selected", true)
                })
            ...
            context.SVG.edges
                ...
                .on("mouseout", function(d, i) {
                    context.SVG.edges.classed("selected", false)
                    context.SVG.nodes.selectAll("rect").classed("selected", true)
                })
            context.SVG.background
                .on("click", function() {
                    context.SVG.edges.style("stroke", "#222").style("opacity", .2)
